...
- (void)getPeopleCount
{
    [AFManager postReqURL:HOME_COUNT_URL reqBody:@{@"lts_id":self.chatId} block:^(id infor)
     {
...
-(void)createBtn{
    ...
    [self.view addSubview:_lebaoNum];
    [self selectLeBaoNum];
    ...
    [self.view addSubview:_peopleNum];
    [self getPeopleCount];
...
{
    [AFManager postReqURL:JIESHUZHIBO reqBody:@{@"id":self.startID} block:^(id infor)
     {
        if ([[NSString stringWithFormat:@"%@",infor[@"code"]] isEqualToString:@"200"])
         {
