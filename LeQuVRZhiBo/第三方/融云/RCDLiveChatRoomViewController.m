...
@property(nonatomic,strong) UILabel *lebaoNum;
        ...
        [[RCIMClient sharedRCIMClient]
         ...
         success:^{
             dispatch_async(dispatch_get_main_queue(), ^{
                 ...
                 [weakSelf.mediaPlayer showMediaPlayer:self.chatId urltype:UrlTypeAuto frame:CGRectNull view:_liveView completion:^(NSInteger defaultNum, NSArray *data)
                 {
...
- (void)initChatroomMemberInfo{
    ...
    _lebaoNum=[[UILabel alloc]initWithFrame:CGRectMake(F_I6_SIZE(10), F_I6_SIZE(70), DEVICE_WIDTH-F_I6_SIZE(100), F_I6_SIZE(20))];
    _lebaoNum.textColor=[UIColor whiteColor];
    _lebaoNum.font=[UIFont systemFontOfSize:F_I6_SIZE(12)];
    [_rightView addSubview:_lebaoNum];
    [self selectLeBaoNum];
    
    _peopleNum = [[UILabel alloc]initWithFrame:CGRectMake(F_I6_SIZE(10), F_I6_SIZE(95), DEVICE_WIDTH-F_I6_SIZE(100), F_I6_SIZE(20))];
    _peopleNum.textColor=[UIColor whiteColor];
    _peopleNum.font=[UIFont systemFontOfSize:F_I6_SIZE(12)];
    [_rightView addSubview:_peopleNum];
    [self getPeopleCount];
...
- (giftView*)giftView
{
    if (!_giftView) {
        ...
        [_giftView.payBtn addTarget:self action:@selector(payMoney) forControlEvents:UIControlEventTouchUpInside];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
    ...
    if ([collectionView isEqual:self.portraitsCollectionView])
    {
        ...
        [cell.portaitView setImageWithURL:[NSURL URLWithString:self.userList[indexPath.row]] placeholderImage:[UIImage imageNamed:@"moren.png"]];
...
#pragma mark-
#pragma mark-查询主播乐宝余额
-(void)selectLeBaoNum
{
    [AFManager getReqURL:[NSString stringWithFormat:@"%@?id=%@",SHOUYI,self.anchorID] block:^(id infor) {
        if ([[NSString stringWithFormat:@"%@",infor[@"code"]] isEqualToString:@"200"])
        {
            _lebaoNum.text=[NSString stringWithFormat:@"乐宝：%@",[infor objectForKey:@"balance"]];
        }
    } errorblock:^(NSError *error)
    {
...
- (void)sendGift
{
    [AFManager getReqURL:[NSString stringWithFormat:SEND_GIFT_URL,[USERDEFAULT valueForKey:@"uid"],self.anchorID,self.giftListDataModel.gid] block:^(id infor)
     {
         if ([[infor objectForKey:@"code"]integerValue]==200)
         {
             ...
             [self sendMessage:giftMessage pushContent:@""];
             [_giftView getMoneyCount];
         }else
         {
             [NSObject wj_showHUDWithTip:@"您的星票不足请先去充值！"];
         }
     } errorblock:^(NSError *error)
     {
...
- (void)getPeopleCount
{
    [AFManager postReqURL:HOME_COUNT_URL reqBody:@{@"lts_id":_targetId} block:^(id infor)
    {
            NSString*total=[infor objectForKey:@"total"];
            _peopleNum.text=[NSString stringWithFormat:@"房间人数：%@",total];
            NSArray*array=[infor objectForKey:@"users"];
      __block  NSInteger i=0;
        for (NSDictionary*dic in array)
        {
            [_userList removeAllObjects];
            [AFManager getReqURL:[NSString stringWithFormat:GET_USER_NICK_URL,dic[@"id"]] block:^(id infor)
             {
                 i+=1;
                 if ([[infor objectForKey:@"code"]integerValue]==200)
                 {
                     [_userList addObject:[infor objectForKey:@"head_pic"]];
                     if (i==array.count)
                     {
